...
def get_note(db: Session, note_id: int) -> schema.Note | None:
    return (db.query(models.Note)
            .filter(models.Note.id == note_id)
            .first())


# TODO add auth for user
...
# TODO add auth for user to create notes
def create_note(db: Session, note: schema.NoteCreate) -> schema.Note:
...
def correct_text(text: str) -> str:
    ...
    for word_dict in speller_result:
        text = text[0:word_dict["pos"]] + word_dict["s"][0] + text[word_dict["pos"]+word_dict["len"]:]
    return text
